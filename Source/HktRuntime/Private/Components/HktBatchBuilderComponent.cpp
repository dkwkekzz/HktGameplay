...
void UHktBatchBuilderComponent::Reset(int32 NumGroups)
{
    ...
    {
        GroupBatches[i].Reset();
        NewbieOwners[i].Reset();
    }
}
...
{
    if (!GroupBatches.IsValidIndex(InGroupIdx))
    {
        GroupBatches.SetNum(InGroupIdx + 1);
    }
...
{
    if (!NewbieOwners.IsValidIndex(InGroupIdx))
    {
        NewbieOwners.SetNum(InGroupIdx + 1);
    }
    return NewbieOwners[InGroupIdx];
}

FHktGroupSimulationState& UHktBatchBuilderComponent::CreateOrGetNewbieState(int32 InGroupIdx)
{
    return NewbieStates.FindOrAdd(InGroupIdx);
}
...
const FHktGroupSimulationState* UHktBatchBuilderComponent::GetNewbieState(int32 InGroupIdx) const
{
    return NewbieStates.Find(InGroupIdx);
}
